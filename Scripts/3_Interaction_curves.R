...
#Create a column of interactions
all_data = all_data %>%
mutate(Unique_interactions = paste0(pollinators,
       "_", plants)) %>%
mutate(Unique_interactions = str_replace_all(Unique_interactions,
      " ", "_"))
#Filter all _sp
#Exclude also apis mellifera
all_data = all_data %>%
filter(!str_detect(Unique_interactions, "_sp")) %>%
filter(!str_detect(Unique_interactions, "Apis_mellifera"))
#Now all interactions are at the species level
...
select(locality, site_id, date, transect,  Unique_interactions)
...
count(date, Unique_interactions) %>%
mutate(n = if_else(n>0,1,0)) %>%
group_by(Unique_interactions) %>%
...
poll_cantavieja_2021 = matrix(c(length(unique(main_cols_cantavieja_2021$date)) ,
          ...
          ncol = 1)
row.names(poll_cantavieja_2021) = c("Plot", poll_result_cantavieja_2021 %>% pull(Unique_interactions))
...
poll_cantavieja_2024 = matrix(c(length(unique(main_cols_cantavieja_2024$date)) ,
          ...
          ncol = 1)
row.names(poll_cantavieja_2024) = c("Plot", poll_result_cantavieja_2024 %>% pull(Unique_interactions))
...
poll_output_cantavieja_2024$iNextEst$size_based
#Plot
...
p1 = ggplot(cantavieja_rarefaction, aes(x = t, y = qD, group = Year)) +
 ...
 scale_y_continuous(limits = c(5, 130),
...
poll_ejea_2021 = matrix(c(length(unique(main_cols_ejea_2021$date)) ,
          ...
          ncol = 1)
row.names(poll_ejea_2021) = c("Plot", poll_result_ejea_2021 %>% pull(Unique_interactions))
...
poll_ejea_2024 = matrix(c(length(unique(main_cols_ejea_2024$date)) ,
          ...
          ncol = 1)
row.names(poll_ejea_2024) = c("Plot", poll_result_ejea_2024 %>% pull(Unique_interactions))
...
poll_output_ejea_2024$iNextEst$size_based
#Plot
...
p2 = ggplot(ejea_rarefaction, aes(x = t, y = qD, group = Year)) +
 ...
 scale_y_continuous(limits = c(5, 200),
      breaks = c(50, ejea_observed$qD,
      ejea_observed$qD, 100))
p2
...
#save plots
saveRDS(p1, "Data/cantavieja_interaction_curves.rds")
saveRDS(p2, "Data/ejea_interaction_curves.rds")
