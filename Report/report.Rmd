...
plot(density(null_canta), xlim=c(min(obs_canta, min(null_canta)), max(obs_canta, max(null_canta))),
     main="Comparison of observed network with null model (Cantavieja)",cex.main=1, las=1)
...
```{r, echo=F,warning=FALSE,dpi=300,out.width="80%",fig.align='center'}
...
#Plot table location 1
canta_table %>%
  kbl("latex",escape=FALSE,col.names = c("\\normalfont{Plants species}", "Frequency"),caption = "\\textbf{Table S1.} Frequencies of the different plant species in Cantavieja") %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "center") %>%
  column_spec(1, italic = T)

#Plot table location 2
ejea_table %>%
  kbl("latex",escape=FALSE,col.names = c("\\normalfont{Plants species}", "Frequency"),caption = "\\textbf{Table S1.} Frequencies of the different plant species in Ejea de los Caballeros") %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "center") %>%
  column_spec(1, italic = T)
